...
- **rem** : 최상위 요소('html')에서 지정한 폰트의ㅣ 크기를 1rem으로 계산
...
- 모둔 요소들은 사각형 박스로 표현되는데 스타일 시트에서는 박스 형태인 요소를 박스 모델이라고 한다.
- CSS를 사용하면 박스 모델 요소의 크기, 위치,속성을 변경할 수 있다.
- 박스 모델은 콘텐트(content) 영역과 테두리(border)와 콘텐츠 영역 사이의 여백인 패딩(padding), 박스 모델 사이의 여백인 마진(margin)으로 구성된다.

## 포지셔닝

### float, clear

- float 속성은 요소의 위치를 왼쪽(left)이나 오른쪾(right)으로 배치하는 속성
...
- relative : 부모 연결해 배치하면서 위치를 지정하여 요소를 배치

...
- 정규표현식 객체를 생성하기 위해서는 정규표현식 리터럴(데이터, 값) 또는 RegExp 생성자 함수를 사용한다.
- 리터럴을 사용하는 것이 일반적이다.
...
- 정규식.test(문자열) : 문자열에서 정규식값과 일치하는 값이 있으면 true, 없으면 false 리턴
...
- 문자열.search(정규식) : 문자열에서 정규식값과 일치하는 값의 시작인덱스값을 리턴 (만일 없다면 -1)
